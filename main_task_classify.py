...
from dataloaders.dataloader_classify import Classify_DataLoader
...
def get_args(description='UniVL on Retrieval Task'):
    ...
    parser.add_argument('--num_cands', type=int, default=100, help='number of candidates')
...
def dataloader_test(args, tokenizer):
    ...
    return dataloader_youcook, len(testset)
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        cand_input_ids, cand_input_mask, video, video_mask, start, end = batch
        if global_step % log_step == 0 and local_rank == 0:
            start_time = time.time()
        loss = model(cand_input_ids, cand_input_mask, video, video_mask,
                     start=start, end=end)
...
def dev_epoch(epoch, args, model, dev_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    global logger
    model.eval()
    log_step = args.n_display
    start_time = time.time()
...
DATALOADER_DICT = {}
DATALOADER_DICT["charades"] = {"train":dataloader_train, "val":dataloader_test}
    ...
    if args.do_train:
        ...
        for epoch in range(args.epochs):
            ...
            if args.local_rank == 0:
                ...
                dev_loss, _ = dev_epoch(epoch, args, model, test_dataloader, device, n_gpu, optimizer,
                                        scheduler, global_step, local_rank=args.local_rank)
                logger.info("The dev loss is: {}".format(dev_loss))
